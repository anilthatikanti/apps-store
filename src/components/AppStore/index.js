...
class AppStore extends Component {
  state = {tabId: tabsList[0].tabId, category: ''}
  ...
  render() {
    const {tabId, category} = this.state
    const filterItem = appsList.filter(item => item.category === tabId)
    const filterApp = filterItem.filter(each =>
      each.appName.toLowerCase().includes(category.toLowerCase()),
    )
    return (
        ...
        <ul className="ulist ">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              tabDetails={eachTab}
              search={this.search}
              tabClassName={tabId === appsList.category}
            />
          ))}
        </ul>
        <hr className="hrLine" />

        <ul className="appsArrange">
          {filterApp.map(eachItem => (
